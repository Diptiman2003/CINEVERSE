...
//   if (!raw) return null;
//   if (raw.toLowerCase() === "null" || raw.toLowerCase() === "undefined") return null;
//   // ← If already a full URL (Cloudinary or any http) return as-is
//   if (/^(https?:\/\/)/.test(raw)) return raw;
//   // ← Remove any cloudinary path prefix without extension
//   if (raw.startsWith("cineverse/")) return null;
//   const cleaned = raw.replace(/^\/?uploads[\\\/]/, "").replace(/\\/g, "/");
//   if (!cleaned) return null;
//   return `${API_BASE_URL}/uploads/${cleaned}`;
// };
...
//   obj.trailerUrl =
//     src.trailerUrl || (lt?.url || lt?.videoId) || null;
...
//   // new: include auditorium in normalized output
...
//     const posterUrl = req.files?.poster?.[0]?.path || req.files?.poster?.[0]?.filename ? (req.files.poster[0].path || getUploadUrl(req.files.poster[0].filename)) : (body.poster || null);
...
//         if (targetArr[idx]) {
//           targetArr[idx].file = filename;
//           targetArr[idx].File = filename;
//         }
//         else targetArr[idx] = { name: "", file: toFilename(filename), File: toFilename(filename) };
//       });
//     };
...
//         { "latestTrailerSchema.title": { $regex: q, $options: "i" } },
...
//       const trailerTypeFilter = { $or: [{ type: "latestTrailers" }, { trim: "latestTrailers" }] };
//       filter = Object.keys(filter).length === 0 ? trailerTypeFilter : { $and: [filter, trailerTypeFilter] };
//     }
...
//     // Debug: log raw filename fields coming from DB to help diagnose missing images
//     try {
//       const rawLog = (items || []).map((it) => ({
//         id: it._id || null,
//         poster: it.poster || null,
//         latestTrailerThumbnail: it.latestTrailer?.thumbnail || null,
//         castFiles: (it.cast || []).map((c) => c?.file || c?.preview || null),
//       }));
//       console.log("RawMoviesFields:", JSON.stringify(rawLog, null, 2));
//     } catch (e) {
//       console.warn("Failed to log raw movie fields", e?.message || e);
//     }
...
//     const { id } = req.params || {};
//     if (!id)
//       return res.status(400).json({ success: false, message: "Required movie ID" });
...
//   return `${API_BASE_URL}/uploads/${raw.replace(/^uploads\//, "")}`;
// };/////////////////////////////made by chat gpt
...
//   ////////////////////////////////////////////////////chatgpt
//   copy.thumbnail = copy.thumbnail ? getUploadUrl(copy.thumbnail) : copy.thumbnail || null;
//   // copy.thumbnail = copy.thumbnail || null;                //made by chatgpt
//   const mapPerson = (p) => {
//     const c = { ...(p || {}) };

//     ////////////////////////////////////////////////// chatgpt
//     c.preview = c.file ? getUploadUrl(c.file) : (c.preview ? getUploadUrl(c.preview) : null);
//     // c.preview = c.file || c.preview || null;                   //made by chatgpt
//     c.name = c.name || "";
//     c.role = c.role || "";
//     return c;
//   };
...
//   obj.poster    = src.poster ? getUploadUrl(src.poster) : null;
//   /////chatgpt
//   // obj.poster = src.poster || null;
//   //obj.poster = src.poster || null;  //////////////////////////2nd chatgpt
//   ////chatgpt
//   obj.thumbnail = lt?.thumbnail ? getUploadUrl(lt.thumbnail) : obj.poster;
