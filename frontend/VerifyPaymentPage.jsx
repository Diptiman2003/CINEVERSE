  ...
  useEffect(() => {
    ...
    const verifyPayment = async () => {
      ...
      if (
        ...
        location.pathname.toLowerCase().endsWith("/cancel")
      ) {
        setStatusMsg('Payment was cancelled. Redirecting to movies...');
        setTimeout(() => navigate('/movies', { replace: true }), 800);
        return;
      }

      if (!session_id) {
        setStatusMsg('No session ID provided in URL');
        return;
      }
      ...
      try {
        ...
        if (res.data?.success) {
          setStatusMsg('Payment successful! Redirecting to home page...');
