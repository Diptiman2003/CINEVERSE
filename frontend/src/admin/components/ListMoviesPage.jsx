...
import { Film, Ticket,Play, Star, Calendar,Play as PlayIcon, Search, X, Clock } from 'lucide-react';

const API_BASE = 'http://localhost:5000'
const FALLBACK_POSTER = `data:image/svg+xml;utf8,${encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" width="640" height="360" viewBox="0 0 640 360"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#140b18" offset="0"/><stop stop-color="#2a0f12" offset="1"/></linearGradient></defs><rect width="640" height="360" fill="url(#g)"/><text x="50%" y="48%" dominant-baseline="middle" text-anchor="middle" fill="#f3f4f6" font-family="Arial, sans-serif" font-size="28">Poster Not Available</text><text x="50%" y="58%" dominant-baseline="middle" text-anchor="middle" fill="#9ca3af" font-family="Arial, sans-serif" font-size="14">Please re-upload image in Add Movie</text></svg>'
)}`;
const FALLBACK_AVATAR = `data:image/svg+xml;utf8,${encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 96 96"><defs><linearGradient id="a" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#1f2937" offset="0"/><stop stop-color="#111827" offset="1"/></linearGradient></defs><circle cx="48" cy="48" r="48" fill="url(#a)"/><circle cx="48" cy="38" r="14" fill="#9ca3af"/><path d="M18 80c6-14 19-21 30-21s24 7 30 21" fill="#9ca3af"/></svg>'
)}`;
...
function safeSrc(maybe) {
  if (!maybe) return null;
  if (typeof maybe === "string") {
     const value = maybe.trim();
...
function onBrokenImage(e) {
  const next = e?.currentTarget;
  if (!next) return;
  if (next.dataset.fallbackApplied === "1") return;
  next.dataset.fallbackApplied = "1";
  next.src = FALLBACK_POSTER;
}

function onBrokenAvatar(e) {
  const next = e?.currentTarget;
  if (!next) return;
  if (next.dataset.fallbackApplied === "1") return;
  next.dataset.fallbackApplied = "1";
  next.src = FALLBACK_AVATAR;
}
...
async function fetchMovies(){
 try {
  ...
  const res = await axios.get(`${API_BASE}/api/movies`, { params });
    ...
    let items = [];
    if (res?.data?.success) {
      // backend may return movies under `items` or `data` depending on route
    ...
    console.table(
      normalized.map((m) => ({
        id: m._id || m.id || null,
        title: m.movieName || m.title || m.displayTitle || null,
        poster: m.poster || null,
        thumbnail: m.thumbnail || null,
        castPreview: (m.cast && m.cast.length && m.cast[0].preview) || null,
      }))
    );
  } catch (err) {
    console.error("FetchMovies Error:", err);
    setError(
      err?.response?.data?.message || err.message || "Failed to load mmovies"
    );
  } finally {
    setLoading(false);
  }
...
function normalizeMovie(item) {
  const source = item?._doc ? { ...item._doc, ...item } : { ...(item || {}) };

  // Clone so we don't mutate original
  const obj = { ...source };
  const latestTrailer = source.latestTrailer || source.latestTrailerSchema || null;

  // Normalize top-level poster
  obj.poster = getImageUrl(source.poster) || (source.poster ? source.poster : null);
  ...
  obj.cast = normalizeTopPeople(source.cast);
  obj.directors = normalizeTopPeople(source.directors);
  obj.producers = normalizeTopPeople(source.producers);
  ...
  if (
    latestTrailer &&
    (source.type === "latestTrailers" ||
      latestTrailer.title ||
      latestTrailer.thumbnail ||
      latestTrailer.videoId)
  ) {
    const lt = latestTrailer || {};

    // Title (trailers may use `title` instead of movieName)
    obj.title = lt.title || source.title || source.movieName || null;
    ...
    obj.thumbnail =
      ...
      getImageUrl(source.thumbnail) ||
      lt.thumbnail ||
      null;

    // trailer link: could be in latestTrailer.videoId or top-level trailerUrl
    obj.trailerUrl = lt.videoId || source.trailerUrl || lt.trailerUrl || null;

    // genres/year/rating/duration/description may live on lt
  obj.genres = lt.genres || lt.geners || source.genres || source.categories || [];
  obj.year = lt.year || source.year || null;
  obj.rating = lt.rating ?? source.rating ?? null;
  obj.duration = lt.duration || source.duration || null;
  obj.description =
    lt.description || source.description || source.story || source.excerpt || null;
  ...
  obj.directors = normalizeLatestPeople(
    lt.directors || latestTrailer?.directors || source.directors || []
  );
  obj.producers = normalizeLatestPeople(
    lt.producers || latestTrailer?.producers || source.producers || []
  );
  obj.singers = normalizeLatestPeople(
    lt.singers || latestTrailer?.singers || []
  );

  } else {
    // For non-latestTrailers, try to normalize thumbnail from other fields if present
    obj.thumbnail = getImageUrl(source.thumbnail) || obj.poster || null;
  }

  // Ensure type is set (some older records may not have type)
  const rawType = obj.type || source.trim || source.movieType || (latestTrailer ? "latestTrailers" : null);
  const mappedType =
    rawType === "latestTrailer"
      ? "latestTrailers"
      : rawType === "comingSoon"
      ? "releaseSoon"
      : rawType;
  obj.type = mappedType || (obj.title && !obj.movieName ? "latestTrailers" : "normal");

  obj.story = obj.story || obj.description || source.story || source.description || source.excerpt || "";
  obj.categories = obj.categories || obj.genres || [];
...
function resolveMovieId(movie) {
  return movie?._id || movie?.id || null;
}
...
async function openDetails(item) {
  ...
  const id = resolveMovieId(normalizedItem);
  if (!id) return;
  ...
  try {
    ...
    const merged = { ...normalizedItem, ...normalizeMovie(res.data.data) };
    setSelected(merged);
  } catch (err) {
          ...
          <aside className={styles5.rightColumn}>
            <div className={styles5.detailSidebar}>
              ...
              {detailLoading && selected ? (
                <div className={styles5.loadingState}>
                  <div className={styles5.loadingText}>Loading details...</div>
                </div>
              ) : selected ? (
  ...
  return (
    <div className={styles5.personGrid}>
      ...
      <div className={styles5.personList}>
        {list.map((p, i) => {
          const src = safeSrc(p.preview || p.file || p.image || p.url || null);
          return (
            <div key={i} className={styles5.personItem}>
              <div className="relative">
                {src ? (
                  <img src={src} alt={p.name || `${roleLabel}-${i}`} className={styles5.personAvatar}
                    onError={onBrokenAvatar}
                  />
                ) : (
                  <img src={FALLBACK_AVATAR} alt={p.name || `${roleLabel}-${i}`} className={styles5.personAvatar} />
                )}
              </div>
...
function DetailView({ item, onClose }) {
  ...
  const displayTitle = item.movieName || item.title || item.displayTitle || "Untitled";
  const storyText = item.story || item.description || item.excerpt || "Story not available.";
  const descriptionText = item.description || item.story || item.excerpt || "Description not available.";
        ...
        {(item.type === "normal" || item.type === "featured") && (
          <>
            <div className="grid grid-cols-1 gap-6">
              <div className={styles5.detailThumbnail}>
                {safeSrc(item.poster) ? (
                  <img src={safeSrc(item.poster)} alt={displayTitle} className={styles5.detailPoster}
                    onError={onBrokenImage}
                  />
                ) : (
                  <div className={styles5.detailPoster} />
                )}
              </div>
