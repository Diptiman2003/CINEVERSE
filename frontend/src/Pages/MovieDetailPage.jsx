...
function normalizeSlotSource(movie) {
  ...
  if (Array.isArray(movie.showtime) && movie.showtime.length) return movie.showtime;
  if (Array.isArray(movie._normalizedSlots) && movie._normalizedSlots.length)
    return movie._normalizedSlots;
  if (Array.isArray(movie.slot) && movie.slot.length) return movie.slot;
  return [];
}
...
export default function MovieDetailPage() {
  ...
  {/* this one div made by ai */}
        {/* Reviews Section */}
      <div className="max-w-5xl mx-auto px-4 pb-16">
        <ReviewSection movieId={movie._id || movie.id || movieIdParam} />
      </div>
